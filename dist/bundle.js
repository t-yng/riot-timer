...
riot.tag2('timer-button', '<button type="button" onclick="{click}">{opts.title}</button>', 'timer-button button,[data-is="timer-button"] button{ font-size: 30px; padding-left: 15px; padding-right: 15px; }', '', function (opts) {
...
riot.tag2('timer', '<span>{createTimeText(time)}</span>', 'timer,[data-is="timer"]{ display: block; } timer span,[data-is="timer"] span{ font-size: 120px; color: white; }', '', function (opts) {
  const self = this;
  this.time = opts.time;
